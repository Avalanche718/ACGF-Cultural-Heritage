...
def voice_synthesis(instrumental_audio, reference_audio, constraint_strength: float,
                     backbone: str = "diffsinger"):
    """
    Step 2: constrained voice reconstruction (paper §3.3).

    backbone:
      - "diffsinger"  : primary model, score/phoneme-driven singing synthesis
      - "fish_speech" : reference-conditioned zero-shot style transfer (needs reference_audio)
      - "cosyvoice"   : reference-conditioned zero-shot style transfer (needs reference_audio)

    constraint_strength bounds how far synthesis can drift from reference_audio's
    timbre/prosody (F0, formants) - this is style-transfer within bounds, not full
    voice cloning. The actual authenticity gating happens in Step 3's discriminator
    loop; this strength value is the *starting* bound passed into that loop.

    NOTE: this function requires third-party packages not in requirements.txt yet.
    See the module docstring at the top of this file for install instructions.
    """
    ...
    if backbone == "diffsinger":
        from diffsinger.inference import DiffSingerSVS  # third_party/DiffSinger
        model = DiffSingerSVS.from_pretrained("diffsinger-base")
        vocals = model.synthesize(
            backing_track=instrumental_audio,
            reference_audio=reference_audio,
            sample_rate=SAMPLE_RATE,
        )
    elif backbone == "fish_speech":
        from fish_speech.inference import FishSpeechS2Pro  # third_party/fish-speech
        model = FishSpeechS2Pro.from_pretrained("fishaudio/fish-speech-s2-pro")
        vocals = model.clone_and_synthesize(
            reference_audio=reference_audio,
            backing_track=instrumental_audio,
            sample_rate=SAMPLE_RATE,
            max_f0_deviation=1.0 - constraint_strength,
            max_formant_deviation=1.0 - constraint_strength,
        )
    elif backbone == "cosyvoice":
        from cosyvoice.inference import CosyVoice2  # third_party/CosyVoice
        model = CosyVoice2.from_pretrained("FunAudioLLM/CosyVoice2-0.5B")
        vocals = model.inference_zero_shot(
            prompt_speech=reference_audio,
            backing_track=instrumental_audio,
            sample_rate=SAMPLE_RATE,
            style_transfer_strength=constraint_strength,
        )
    else:
...
def acgf_pipeline(text_prompt: str, reference_audio=None, constraint_strength: float = 0.8,
                   ...
                   voice_backbone: str = "diffsinger", discriminator=None):
    """
