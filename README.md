...
|---|---|---|---|
| **Fish Speech (S2 Pro)** | commit `e5e2926` (2026-06-09), `pyproject.toml` version `2.0.0` | **Wired up** — `voice_synthesis(backbone="fish_speech")` calls the real `TTSInferenceEngine`, `launch_thread_safe_queue`, `load_decoder_model`, `ServeTTSRequest`/`ServeReferenceAudio` classes, confirmed to exist at those exact import paths. | Checkpoint paths (`checkpoints/fish-speech-s2-pro`) are placeholders you fill in after downloading `fishaudio/s2-pro` from Hugging Face — the function calls themselves are real, not fabricated. |
